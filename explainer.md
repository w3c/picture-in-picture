...
partial interface HTMLVideoElement {
  ...
  Promise<CustomAction> addPictureInPictureAction(CustomActionMetadata action);
...
interface CustomActionMetadata {
  ...
  callback? fn; // Handles action.

  void setHandler(callback fn);
};
...
<script>
  ...
  const actions = {
    label: 'Skip',
    artwork: [
      { src: 'image_1.png', sizes: '96x96', type: 'image/png' },
      { src: 'image_2.png', sizes: '128x128', type: 'image/png' },
    ],
  };

  const action = await video.addPictureInPictureAction(action);
